...
const FilterPanel = ({ isOpen, onClose, onApplyFilters, cities, departments, roles, currentFilters, onResetFilters }) => {
...
const panelRef = useRef(null);
  ...
  const handleReset = (filter1, filter2, filter3) => {
    setFilters((prevFilters) => ({
      ...prevFilters,
      [filter1]: "",
      [filter2]:"",
  ...
  return (
      <div className="filter-panel" ref={panelRef}>
        ...
        <h2>Фильтры</h2>
        <button onClick={() => handleReset('city', 'role', 'department')} className="reset-filters-button">Сбросить фильтры</button>
        </div>
          <label>
          Город:
